...
create table object (
	...
	index_state number default 0 not null,
	index_tries number default 0 not null,
...
create index idx_object_index_state on object(index_state);
...
create index idx_index_word on index_word(word);
...
create index idx_index_word_object on index_word_object(object_id);
...
select
  ...
  (select sqrt(frequency)*
          (select power(1+log(sub_score/(select (count(*)+1.0) from index_word_object where word_id=iwo.word_id)), 2) from (select count(*) as sub_score from object where index_state=1))*
          (select (1/(sqrt(count(*))+0.0)) from INDEX_WORD_OBJECT where object_id=iwo.object_id) from index_word_object where word_id=iwo.word_id and object_id=iwo.object_id
  ) as score
...
--  iwo.word_id as word_id,
--  (select sqrt(frequency)*
--          (select power(1+log(10, sub_score/(select (count(*)+1.0) from index_word_object where word_id=iwo.word_id)), 2) from (select count(*) as sub_score from object where index_state=1))*
--          (select (1/(sqrt(count(*))+0.0)) from INDEX_WORD_OBJECT where object_id=iwo.object_id) from index_word_object where word_id=iwo.word_id and object_id=iwo.object_id
--  ) as score
--	
--from index_word_object iwo;

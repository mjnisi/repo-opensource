...
select
  ...
  --iwo.property_id as prop_id,
  --iwo.frequency as freq,
  --iwo.obj_prop_type_id,
  nvl2(iwo.property_id,
    -- metadata
    (
      select (
        sqrt(frequency)* --tf
        (
          ...
          (
            ...
            from
            object o join property p on p.object_id=o.id and o.index_state_metadata=1 and p.object_type_property_id=object_type_property_id*/
            select count(*) as sub_score
    ...
    (
      select (
        sqrt(frequency)* --tf
        (
          select power(1+log(10, sub_score/(select (count(*)+1.0) from index_word_object where word_id=iwo.word_id and property_id is null)), 2)
          from (select count(*) as sub_score from object where index_state_content=1)
        )* --idf
        --(select (1/sqrt(sum(frequency))) from index_word_object iwo0 where iwo0.object_id=object_id and iwo0.property_id is null)* --lengthNorm
        (select (1/sqrt(sum(frequency))) from index_word_object iwo0 join object o on iwo0.object_id=o.id and o.index_state_metadata=1 where iwo0.object_id=iwo.object_id and iwo0.property_id is null)*
        (1) --boost
      ) from index_word_object where word_id=iwo.word_id and object_id=iwo.object_id and property_id is null
    )
  ) as score
